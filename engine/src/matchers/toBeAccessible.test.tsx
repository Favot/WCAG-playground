...
const Button = () => (
  <TouchableOpacity accessible accessibilityRole="button" accessibilityLabel="Like">
    <Image source={Icons.filledHeart["32px"]} />
  </TouchableOpacity>
);
...
it("should always force returnViolations to be true", () => {
  ...
  expect(() => expect(<NonAccessibleButton />).toBeAccessible()).not.toThrow(AccessibilityError);
  expect(() =>
...
it("should allow configurable rules for a single run of jest matcher", () => {
  expect(() => expect(<NonAccessibleButton />).toBeAccessible({ rules: [] })).not.toThrow();
  expect(() =>
